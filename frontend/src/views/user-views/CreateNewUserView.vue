    ...
    <div class="d-flex justify-content-center align-items-center vh-100 bg-dark text-white">
        <BForm class="d-flex flex-column justify-content-center align-items-center" @submit="onSubmit" @reset="onReset">
            ...
            <BFormGroup id="input-group-3" label="Senha:" label-for="input-2" class="mb-4">
                <BFormInput id="input-3" v-model="form.password" type="password" placeholder="Digite a senha" required />
            </BFormGroup>

            <BFormGroup id="input-group-3" label="Confirmar Senha:" label-for="input-4" class="mb-4">
                <BFormInput id="input-4" ref="confirmarInput" v-model="confirmPassword" @input="validarSenha" type="password" placeholder="Confirme a senha" required />
            </BFormGroup>

            <p class="text-danger" v-if="validarSenha()">Senhas não coincidem</p>

            <div class="d-flex flex-row">
                <BButton type="submit" variant="primary" class="mr-2" :disabled="!(form.name && form.email && form.password && confirmPassword)">Criar</BButton>
                <BButton type="reset" variant="danger" :disabled="!(form.name || form.email || form.password)">Limpar</BButton>
            </div>
